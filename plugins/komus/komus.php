    ...
    case 'step':
        ...
        if (empty($contact_id) || empty($call_id)) {
            ...
            if (!empty($_POST)) {
                ...
                if (!empty($form_id)) {
                    $sql_forms_string = "SELECT ff.* FROM {$db_x}komus_forms_fields ff
            ...
            $sql_form_string = "SELECT f.id, f.title, ff.type, ff.title, ff.name, ff.required, c.if_field_value, c.to_node
                                ...
                                WHERE f.node_id = $node_id ORDER BY ff.sort, ri.sort";
            $sql_form = $db->query($sql_form_string);
            if ($sql_form->fetchColumn() > 0) {
                $sql_fields_string = " SELECT ff.*, f.title form_title FROM cot_komus_forms_fields ff 
...
HTML;
            ...
            $age_confirm = ($call_data['client_age'] < 25 && $call_data['client_age'] > 71) ?
                $L['komus_refusal'] : $L['komus_confirm'];
