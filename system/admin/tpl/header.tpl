...
<body>
	...
	<ul id="user" class="body">
		<li id="hi">{PHP.L.hea_welcome}, <a href="users.php?m=profile">{PHP.usr.name}</a><span class="spaced">{PHP.cfg.separator}</span><a href="pm.php"><!-- IF {PHP.usr.messages} == 0 -->{PHP.L.Private_Messages}<!-- ELSE -->New messages: {PHP.usr.messages}<!-- ENDIF --></a><span class="spaced">{PHP.cfg.separator}</span>{PHP.out.loginout}</li>
